...
- **Display order is separate:** Display order in the format switcher tabs is Blaster, Mega, Hobby, Jumbo, Breaker Delight (left to right) — this is independent of parent priority. Display order is least-to-greatest by price and odds, which matches how consumers naturally scan options. Two ordering rules exist (parent priority and display order); they should not be conflated.
...
- **Saw:** Sports cards do not have a sanctioned bulk download endpoint — only per-set CSV downloads via website URLs. Rate limit is hard: 1 CSV per 10 minutes per API documentation (vendor email above said "1 per 5 minutes" — the API docs are authoritative; the email number was incorrect), which translates to 6/hour or 144/day if run continuously. The vendor's framing is technical-mechanism-only — they did NOT address commercial use of the data as the silent backend of a paid analytics product.
...
**Format list note (related, not strictly schema):** Beta format scope locked at Blaster, Mega, Hobby, Jumbo, Breaker Delight (display order, left to right — least-to-greatest by price and odds, revised May 2026 from prior order). Retail dropped from beta scope, deferred as a post-beta addition (PRE-BETA-CHECKLIST.md #13.1). Schema-compatible — `box_format` is VARCHAR — no migration needed when Retail is added back.
