...
- Schema amendments queued for after Step 4: per-parallel pull rates (currently using category-representative rates, EV math is approximate), guaranteed pulls structure (JSON column vs separate table)
...
- **Guaranteed pulls per box — FEATURE DECIDED, structural choice OPEN (May 2026):** Each format has manufacturer-stated guaranteed pulls — Hobby = 1 auto guaranteed, Jumbo = 3 autos, Blaster = 2 Sepia + 2 Pink Refractors, etc. Guaranteed pulls have probability = 1.0 within the box and are conceptually a special case of pull rates. Structural choice is open: JSON column on box_sets (fast, flexible, not queryable) vs. separate box_guarantees table (cleaner relational model, queryable, more rows to maintain). UI placement also open — likely in or near the Pull Rates section. Schema amendment queued for a focused session before full seed. Not blocking Step 4 — the existing pipeline works end-to-end without it. Tracked further in PRE-BETA-CHECKLIST as a new schema amendment item (to be added when schema amendment session is planned).
...
- **Spreadsheet format:** One Excel file, one tab per schema table. Sport and year are columns within the `box_sets` tab — not separate files. Each format (Breaker, Jumbo, Hobby, Mega, Blaster) is a separate row in `box_sets`, linked by `parent_set_id`.
