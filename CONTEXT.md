...
72. ✅ BoxProfilePageV2 built at `/box-v2/:slug` — separate POC page with player-first layout. Section order: Format Switcher → Hero (Market Price · MSRP · ROI) → Top Chases (Top Players | Top Parallels tabs) → Grails (standalone) → Pull Rates → Inserts → Price Trends (Box price | Grailed tabs) → Full Checklist (no per-card pricing). V1 at `/box/:slug` is untouched.
