...
- **Routing:** React Router. All routes defined in App.jsx. Routes: `/` (homepage), `/browse` (browse page with filters), `/box/:slug` (box profile — renders V3 by default), `/box-v1/:slug` (V1 — nav tab only), `/box-v2/:slug` (V2 POC — nav tab only), `/box-v3/:slug` (V3 — same as `/box/:slug`), `/about`, `/news`, `/contact`, `/help`, `/signin`, `/signup`, `/check-email`.
...
72. ✅ BoxProfilePageV2 built at `/box-v2/:slug` — separate POC page with player-first layout. Section order: Format Switcher → Hero (Market Price · MSRP · ROI) → Top Chases (Top Players | Top Parallels tabs) → Grails (standalone) → Pull Rates → Inserts → Price Trends (Box price | Grailed tabs) → Full Checklist (no per-card pricing).
73. ✅ Full checklist for 2023 Topps Chrome Baseball generated from 21 SCP pricing CSVs via `scripts/generate_checklist.py` — 12,657 cards across 5 tiers (Premium Hits 542, Autographs 1,844, Refractors 8,065, Inserts & Short Prints 1,986, Base & Rookies 220). Output is `src/utils/checklistTiers2023ToppsChrome.js` — **auto-generated, DO NOT EDIT BY HAND**. Re-run script to regenerate from updated CSVs.
74. ✅ Inserts section added to BoxProfilePageV2 — expandable accordion showing all 12 insert sets (TacoFractor, Youthquake, Future Stars, Let's Go!, Radiating Rookies, Exposé, In Technicolor, Titans, 1988 Chrome, Ultraviolet All-Stars, Short Print, MVP Refractor Buybacks). Each row shows player count, parallel tags, and expands to full player list with card numbers. `InsertSetsSection.jsx` + `InsertSetsSection.css` in `src/components/`.
75. ✅ Price Trends section combined in V2 — single section with Box price | Grailed tab switcher. Box price tab shows sealed box market trend (PriceTrendChart); Grailed tab shows average market price of grailed cards over 8 weeks (TierPriceTrendChart with DUMMY_GRAIL_PRICE_TREND mock data). Replaces two separate sections.
76. ✅ Top Chases section finalized in V2 — two tabs: Top Players (ranked list with RC badge, team, position) and Top Parallels (rarest parallel/insert by format odds, exact oddsDenominator from ODS source data). Grails is a standalone section below Top Chases.
77. ✅ BoxProfilePageV3 built at `/box-v3/:slug` — V2 layout (3-metric hero, Real ROI, Inserts) with V1-style combined Top Chases/Grails section (single section, two tabs). Top Chases uses real ODS data: top 7 rarest unpriced cards (Future Stars Autographs SuperFractors /1, 1 in 2,472,912 hobby odds). Displayed via TopChaseRow with correct 1/1 + Auto + RC badges and Not in Circulation.
78. ✅ V3 set as default box profile — `/box/:slug` now renders BoxProfilePageV3. V1 moved to `/box-v1/:slug`. All home page and browse links automatically use V3. V1/V2/V3 each accessible via dedicated nav tab (V1 Preview, V2 Preview, V3 Preview).
79. ✅ ODS source-of-truth data file added to repo — `data/2023 topps chrome baseball/2023-topps-chrome-baseball_from_scp.ods`. Three sheets: box_sets (5 rows), cards (10,266 rows), pull_rates (558 rows). Read with `pd.read_excel(path, engine='odf')`. This is the canonical data source — do not use CSVs as primary.
...
- ⚠️ DUMMY_TIER_TREND_DATA bypasses the hook and is consumed inline in BoxProfilePage.jsx and BoxProfilePageV3.jsx — at database phase this needs to move into useBoxProfile in both files. Refactor task, not just a data swap.
- ⚠️ DUMMY_FORMAT_DATA drives formatData in BoxProfilePage.jsx and BoxProfilePageV3.jsx — needs to come from useBoxProfile once parent_set_id is live on box_sets. Refactor task in both files.
- ⚠️ MOCK_V3_TOP_CHASES in boxProfileMockData.js is a hardcoded stand-in for V3's Top Chases. At database phase, replace with real ODS/API data via useBoxProfile — the shape (playerName, variationName, printRun, isAutograph, rookieCard, price) already matches TopChaseRow's expected props.
...
- ⚠️ `src/utils/checklistTiers2023ToppsChrome.js` is auto-generated — DO NOT EDIT BY HAND. Re-run `python3 scripts/generate_checklist.py` from the project root to regenerate from updated SCP CSVs. CSVs must be in `data/2023 topps chrome baseball/2023 Topps Chrome Pricing Csv/`. The generated file is 3.6MB and causes a Vite bundle size warning — this is expected and not an error.
- ⚠️ `src/components/GrailedPrices.jsx` and `GrailedPrices.css` are dead code — created early in the V2 build and superseded by GrailsTab + the combined Price Trends section. Delete before Pro audit #1. See PRE-BETA-CHECKLIST.md.
