...
### Format Switcher
Tab row at the top of the box profile page. Switches between available formats. Display order, left to right: Blaster, Mega, Hobby, Jumbo, Breaker Delight. This order is least-to-greatest by price and odds, which matches how consumers naturally scan options when comparing value. (Parent identity rule is separate — see schema docs.) Only shows formats that exist for the current set. Switching format updates MSRP, pull rates, EV, and ROI. Checklist cards stay the same — only odds and pricing change. URL updates via query parameter: `/box/slug?format=hobby`. Requires `parent_set_id` on `box_sets` table (database phase). Built with dummy data — wire to real data during database phase. No horizontal scroll on mobile — tabs use `flex: 1` to distribute evenly across full width.
